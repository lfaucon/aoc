...
intersection_points_indices[END] = len(intersection_points)
...
def walk(path):
    while not path[-1] in intersection_points_set and path[-1] != END:
        lr, lc = path[-1]
...
for intersection_idx in range(len(intersection_points)):
    ...
    for (pr, pc), slope in zip([(ir, ic+1), (ir, ic-1), (ir+1, ic), (ir-1, ic)], "><vZ"):
        print(forest[pr][pc], slope)
        if forest[pr][pc] == slope:
...
while queue:
    ...
    for next_position, distance_increment in connections[position].items():
        queue.append((next_position, distance + distance_increment))
